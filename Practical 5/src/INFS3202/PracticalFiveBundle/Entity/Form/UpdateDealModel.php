...
namespace INFS3202\PracticalFiveBundle\Entity\Form;
...
class UpdateDealModel extends ContainerAware {

    public function setDeal($deal){
        if($deal != null){
            $this->setDealId($deal->getId());
            $this->setDealTitle($deal->getTitle());
            $this->setDealDescription($deal->getDescription());
            $this->setDealPrice($deal->getPrice());
            $this->setDealBanner($deal->getBanner());
            $this->setDealStartDate($deal->getTimestampStart());
            $this->setDealEndDate($deal->getTimestampEnd());
            $this->setCategoryName($deal->getCategory()->getName());
            $this->setProprietorName($deal->getProprietor()->getName());

            $reviewRepository = $this->container->get('doctrine')->getRepository('INFS3202PracticalFiveBundle:Review');
            $reviews = $reviewRepository->findBy(['deal' => $deal]);

            $reviewsText = '';

            foreach($reviews as $review){
                $reviewsText .= sprintf("%s|%s|%s\r\n", $review->getAuthor(), $review->getTitle(), $review->getDescription());
            }

            $this->setReviews($reviewsText);
        }
    }
    ...
    public function validate(ExecutionContextInterface $context)
    {
        ...
        $existingDeal = $em->getRepository('INFS3202PracticalFiveBundle:Deal')->findOneBy(['title' => $this->getDealTitle()]);

        if($existingDeal != null && $existingDeal->getId() != $this->getDealId()){
